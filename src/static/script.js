...
function initializeSocket() {
    ...
    socket.on('shower_started', function(data) {
        if (data.user_name !== currentUser) {
            updateShowerStatus('Ocupado', `${data.user_name} está usando (${data.duration} min)`);
...
function updateShowerStatus(status, details) {
    ...
    statusDisplay.style.borderLeftColor = status === 'Disponível' ? '#4CAF50' : 
                                         status === 'Em uso' ? '#667eea' : '#FF9800';
}
